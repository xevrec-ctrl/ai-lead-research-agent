...
class Editor:
    ...
    def __init__(self) -> None:
        self.llm = create_chat_model("report", streaming=True)
    ...
    async def compile_content(
        ...
        if assessment := state.get("opportunity_assessment"):
            combined_content += "\n\n客户机会与 Agent 方案分析：\n"
            combined_content += json.dumps(assessment, ensure_ascii=False, indent=2)
