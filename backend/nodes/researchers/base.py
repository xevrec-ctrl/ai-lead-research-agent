...
from ...config import Settings, create_chat_model
...
class BaseResearcher:
    def __init__(self):
        ...
        self.llm = create_chat_model("research", settings=settings, streaming=True)
    ...
    async def generate_queries(self, state: Dict, prompt: str):
        ...
        client_need = state.get("client_need", "Not provided")
        ...
        try:
            ...
            query_prompt = ChatPromptTemplate.from_messages(
                [
                    (
                        "system",
                        "You are researching {company}, a company in the {industry} industry, headquartered in {hq_location}. Generate search queries only; do not answer the research question.",
                    ),
                    (
                        "user",
...
{format_guidelines}""",
            ...
            async for chunk in chain.astream(
                ...
                if job_id:
                    try:
                        logger.info(
                            f"job_id={job_id}, job_id in job_status={job_id in job_status}"
                        )
                        if job_id in job_status:
                            ...
                            logger.warning(
                                f"job_id {job_id} not found in job_status. Available keys: {list(job_status.keys())[:3]}"
                            )
            ...
            raise RuntimeError(
                f"Fatal API error - query generation failed: {str(e)}"
            ) from e
    ...
    async def search_documents(self, state: ResearchState, queries: List[str]):
        ...
        search_tasks = [
            self.tavily_client.search(query, **search_params) for query in queries
        ]
